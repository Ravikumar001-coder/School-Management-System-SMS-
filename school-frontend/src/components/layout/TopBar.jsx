...
const TopBar = ({ onMobileMenuClick, onDesktopMenuToggle, onSearchClick }) => {
  const { user } = useAuth();
        ...
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="relative hidden md:flex items-center max-w-sm w-full">
            ...
            <input 
              ...
              readOnly
              className="w-full bg-slate-50 border-none rounded-xl pl-10 pr-4 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 outline-none transition-all cursor-pointer"
            />
          </div>
        </div>
